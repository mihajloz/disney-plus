...
import brandBg from "../../../public/assets/img/brands/pixar-bg.jpeg";
...
async function BrandPixar() {
  const fetchedMovies = await getMovies();

  const pixarMovies = fetchedMovies.filter(
    (movie: any) => movie.Brand === "Pixar"
  );

  const featured = pixarMovies.sort((a: any, b: any) => (b.Featured ? 1 : -1));

  const featured2 = pixarMovies.slice(2, 13);

  const originals = [...pixarMovies].reverse().slice(0, 10);
      ...
      <div>
        <div className="flex flex-col absolute w-full">
          ...
          <div className="fixed bg-image brand-bg-img">
            <Image src={brandBg} alt="Pixar" className="w-[100vw]" />
