...
import brandBg from "../../../public/assets/img/brands/star-bg.jpeg";
...
async function BrandStar() {
  const fetchedMovies = await getMovies();

  const starMovies = fetchedMovies.filter(
    (movie: any) => movie.Brand === "Star"
  );

  const featured = starMovies.sort((a: any, b: any) => (b.Featured ? 1 : -1));

  const featured2 = starMovies.slice(2, 13);

  const originals = [...starMovies].reverse().slice(0, 10);
